...
void RikiuotiStudentus(deque<Studentas>& studentai, const string& vargsiukaiFailas, const string& kietiakiaiFailas, int pazymiuKiekis) {
    ...
    std::chrono::steady_clock::time_point sortEnd = std::chrono::steady_clock::now();
        cout << "Failo rikiavimo i dvi grupes su pradinio deko istrinimu laikas: " << fixed << setprecision(6) << std::chrono::duration_cast<std::chrono::duration<double>>(sortEnd - sortBegin).count() << " sekundes." << endl << endl;
    ...
    if (vargsiukaiStream.is_open() && kietiakiaiStream.is_open()) {
        ...
        cout << "Klaida: Nepavyko atidaryti failu " << vargsiukaiFailas << " ir " << kietiakiaiFailas
             <<
             endl;
    }
}
